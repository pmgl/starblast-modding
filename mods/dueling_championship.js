...
var instructor_duration = 10; // duration for the instructor stays in the screen (in seconds)
...
var modUtils = {
  ...
  ship_infos.forEach((info, i) => {
    let nspace = new Array(maxNamelength - info.name.length).fill(" ").join(""), dspace = new Array(maxDesignerlength - info.designer.length).fill(" ").join(""), compo = [
      {type: "box", position: [10,0,80,100], fill: "hsla("+(180/ship_infos.length*(2*i+1))+",100%,50%,1)"},
      {type: "text", position: [10,0,80,100], value: nspace + info.name + nspace, color: black}
    ];
    (ship.custom.type === info.code) && compo.push({type: "text", position: [10,0,80,40], value: "    ✓    ", color: black});
    (info.designer) && compo.push({type: "text", position: [10,60,80,40], value: dspace + "by: " + info.designer + dspace, color: black});
    sendUI(ship, {
      id: "s"+info.code,
  ...
  sendUI(ship, {
    ...
    visible: !bool&&isActive&&ship.custom.shipped,
    clickable: false,
    position: [35,35,30,45],
    components: [
      {type: "text", position: [0,90,100,10], value: "Press [ for previous and ] for next ship", color: dfl_tcl},
      {type: "round", position: [0,0,100,30/45*100], fill: "hsla(0, 0%, 0%, 0)", width: 1, stroke: dfl_tcl}
    ]
  });
  sendUI(ship, {
    id: "map",
  ...
  ship_infos.length > 1 && sendUI(ship, {
    ...
    components: [
      ...
      { type: "text",position:[0,10,100,40],value:ship.custom.shipped?"Close":"Change ship",color:black},
  ...
  if (ship != null) {
    if (ship.custom.spectate) ship.set({type: spectator, crystals: 0});
      ...
      ship.set({type: ship.custom.type,crystals: crystals,shield: 1e4});
    }
    ship.set({stats:88888888, collider: !ship.custom.spectate});
  }
  ...
  if (ship != null) {
    ...
    if (ship.custom.pendingTp === -1) {
      let angle = 0, distance = 0, noded = node !== lobby && !ship.custom.spectate;
      if (noded) {
        angle = angles[node] || 0;
...
this.event = function(event, game) {
  ...
  if (ship != null) switch (event.name) {
    case "ui_component_clicked":
      let id = event.id;
      if (ship.custom.currentNode === lobby) switch (id) {
        case "chooser":
        ...
        case "map":
          ...
          break;
        case "snext":
          if (ship.custom.shipped && !ship.custom.spectate) max(ship, (ship_infos[ship_infos.findIndex(t => t.code === ship.custom.type)+1]||ship_infos[0]).code);
          break;
        case "sprev":
          if (ship.custom.shipped && !ship.custom.spectate) max(ship, (ship_infos[ship_infos.findIndex(t => t.code === ship.custom.type)-1]||ship_infos[ship_infos.length-1]).code);
          break;
        case "nprev":
          if (ship.custom.mapped && ship.custom.spectate && !ship.custom.ready) {
            let t = grids.map(i => dist(ship.x, ship.y, ...i)), min = Math.min(...t), node = t.indexOf(min);
            setNode(ship, (node>0?node:t.length)-1);
          }
          break;
        case "nnext":
          if (ship.custom.mapped && ship.custom.spectate && !ship.custom.ready) {
            let t = grids.map(i => dist(ship.x, ship.y, ...i)), min = Math.min(...t), node = t.indexOf(min);
            setNode(ship, (node<(t.length-1)?node:-1)+1);
          }
          break;
        default:
          if (id.match(/^n\d+$/) && ship.custom.spectate && !ship.custom.ready && ship.custom.mapped) setNode(ship, Number(id.slice(-id.length+1)));
          if (id.match(/^s\d+$/) && !ship.custom.spectate && ship.custom.shipped) {
    ...
    case "ship_spawned":
      ship.custom.pendingTp = -1;
      setNode(ship, lobby);
      max(ship, (ship_infos.find(info => info.code === ship.custom.type)||ship_infos[0]).code);
      break;
    default:
      break;
  }
