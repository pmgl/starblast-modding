...
* Shipbuilders: Thuliux (Eggsagon), Nova, SChickenMan, Neuronality and others

Season 2 (v1.0.1) - Main version
...
var ship_infos = ships.map(i => {
  let t = JSON.parse(i);
  return {name: t.name, designer: t.designer||"", code: t.typespec.code}
...
var modUtils = {
  ...
  if (ship != null) {
    type = type || ship.type;
    if (ship_infos.find(info => info.code === type)) {
    ...
    case "ui_component_clicked":
      ...
      if (ship.custom.currentNode === lobby) switch (id) {
        ...
        case "map":
          ...
          break;
        case "snext":
          if (ship.custom.shipped && !ship.custom.spectate) max(ship, (ship_infos[ship_infos.findIndex(t => t.code === ship.type)+1]||ship_infos[0]).code);
          break;
        case "sprev":
          if (ship.custom.shipped && !ship.custom.spectate) max(ship, (ship_infos[ship_infos.findIndex(t => t.code === ship.type)-1]||ship_infos[ship_infos.length-1]).code);
          break;
        case "nprev":
